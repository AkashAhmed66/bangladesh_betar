...
>
    <div>
        <p class="form-label">{{ __($label) }} @if ($required)<span class="text-rose-500" aria-hidden="true">*</span>@endif</p>
        <p class="form-help">{{ __($help) }}</p>
    </div>
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center">
        <div class="relative {{ $wide ? 'h-32 w-full max-w-sm' : 'size-32 shrink-0' }} overflow-hidden rounded-xl border border-dashed border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-900">
            <img x-show="preview" x-cloak :src="preview" alt="{{ __($label) }} {{ __('preview') }}" class="size-full object-cover">
            <div x-show="! preview" class="flex size-full flex-col items-center justify-center gap-2 text-slate-400">
                ...
                <span class="text-xs">{{ __('No image selected') }}</span>
            </div>
        </div>
        ...
        <div class="flex flex-col items-start gap-2">
            <label for="{{ $inputId }}" class="btn-secondary cursor-pointer">
                ...
                <span x-text="preview ? @js(__('Replace image')) : @js(__('Choose image'))"></span>
            </label>
            <input
                id="{{ $inputId }}"
                name="{{ $name }}"
