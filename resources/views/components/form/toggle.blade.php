...
<label class="flex cursor-pointer items-start gap-3">
    ...
    <span>
        <span class="block text-sm font-medium text-slate-700 dark:text-slate-300">{{ __($label) }}</span>
        @if ($help)<span class="block text-xs text-slate-500 dark:text-slate-400">{{ __($help) }}</span>@endif
    </span>
</label>
