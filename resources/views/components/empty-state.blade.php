...
<div class="flex flex-col items-center justify-center px-6 py-16 text-center">
    ...
    <p class="mt-4 text-sm font-medium text-slate-700 dark:text-slate-300">{{ __($title) }}</p>
    @if ($message)
        <p class="mt-1 max-w-sm text-sm text-slate-500 dark:text-slate-400">{{ __($message) }}</p>
    @endif
    @if (trim($slot) !== '')
