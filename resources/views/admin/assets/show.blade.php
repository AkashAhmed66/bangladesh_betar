...
<div class="mb-6 flex flex-wrap items-start justify-between gap-4">
    ...
    <div class="flex flex-wrap gap-2"
        ...
        @endcan
        @can('assets.publish')
            @if (! $asset->isArchived() && in_array($asset->status, ['approved', 'unpublished'], true) && $asset->rights_status === 'approved')
...
@if ($asset->isArchived())
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-(--radius-app) border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
        <span class="flex items-center gap-2">
            <x-icon name="archive" class="size-4 shrink-0" />
            This asset is archived and unpublished. Restore it from Archive before publishing it again.
        </span>
        <a href="{{ route('admin.archive.index') }}" class="font-semibold underline underline-offset-2">Open Archive</a>
    </div>
@endif
    ...
    <div class="bg-slate-900 p-6 dark:bg-slate-950"
        ...
        @if ($playVersion)
            <audio x-ref="audio" preload="none" class="hidden"
                   data-hls="{{ \App\Support\Hls::adminAssetHls($asset, $playVersion) ?? '' }}"
                   data-fallback="{{ route('admin.assets.stream', ['asset' => $asset->id, 'version' => $playVersion->id], false) }}"
...
<div class="grid grid-cols-1 gap-6 xl:grid-cols-3">
    <div class="space-y-6 xl:col-span-2">
        ...
        {{-- The external analysis runs asynchronously. Poll only while it is pending and
             replace this trusted server-rendered fragment when the result is saved. --}}
        @php
            $analysisJob = $asset->latestAiAnalysisJob;
            $analysisPending = $asset->status === 'analyzing' || $analysisJob?->status === 'processing';
        @endphp
        <div x-data="aiAnalysisMonitor(@js(route('admin.assets.analysis-status', $asset)), @js($analysisPending))">
            <div x-ref="content">
                @include('admin.assets.partials.ai-analysis', [
                    'job' => $analysisJob,
                    'pending' => $analysisPending,
                ])
            </div>
            <p x-show="error" x-text="error" x-cloak
               class="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-700 dark:bg-amber-500/10 dark:text-amber-300"></p>
        </div>
